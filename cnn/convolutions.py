...
# construct the Sodel x-axis kernel
sobelX = np.array((
    [-1, 0, 1],
    [-2, 0, 2],
    [-1, 0, 1]), dtype="int")

# construct the Sodel y-axis kernel
sobelY = np.array((
    [-1, -2, -1],
    [0, 0, 0],
    [1, 2, 1]), dtype="int")
...
for (kernelName, K) in kernelBank:
    ...
    '''
    #cv2.imwrite("output/{}-convolve.jpg".format(kernelName), convolveOutput)
    cv2.imwrite("output/{}-opencv.jpg".format(kernelName), opencvOutput)
