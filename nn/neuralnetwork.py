...
class NeuralNetwork:
    def __init__(self, layers, alpha=0.1):
        # layers: list of integer which represents the actual architecture of the feedforward network
        # alpha: learning rate of the nn using during weight update phase
        ...
        for i in np.arange(0, len(layers) - 2):
            ...
            w = np.random.randn(layers[i] + 1, layers[i + 1] + 1)  # +1 for bias
            self.W.append(w / np.sqrt(layers[i]))

        # the last 2 layers are a special case where the input
    ...
    @staticmethod
    def sigmoid_deriv(x):
        # compute the derivation of the sigmoid function
        # ASSUMING that 'x' has already been passed through the 'sigmoid' function
        return x * (1 - x)
    ...
    def fit_partial(self, x, y):
        # x: individual data point from our design matrix
        # y: the corresponding class label
        # construct our list of output activations for each layer
        # as our data point flows through the network; the first
        # activation is a special case -- it's just the
        ...
        # BACKPROPAGATION
        # the first phase of backpropgation is to compute the
        ...
        # already have taken them in to account)
    ...
    def predict(self, X, addBias=True):
        ...
        for layer in np.arange(0, len(self.W)):
            ...
            # then passing this value though a nonlinear activation function
    ...
    def calculate_loss(self, X, targets):
        # make predictions for the input data points when compute the loss
